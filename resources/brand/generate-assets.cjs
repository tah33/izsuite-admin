 ...
 * palette. The wordmark is outlined from Outfit ExtraBold (vendored in
 * public/fonts), so the SVGs are pure paths with no font dependency at render
 * time - which is what lets the same artwork work in email clients and the PDF
 * renderer, neither of which can load a webfont.
 *
 * The design is shared with the Nuxt frontend at ../izsuite-nuxt, which draws
 * the same lockup live in CSS (components/BrandLogo.vue). Two rules keep the
 * two in step:
 *   - the typeface and palette here must match assets/css/main.css there;
 *   - the "iz" monogram is geometry, not outlined type (see GLYPH below), so
 *     both projects carry byte-identical shapes without sharing a font.
 *
...
// Keep in step with --brand-blue / --brand-navy in the Nuxt project's
// assets/css/main.css, config/brand.php, and resources/css/app.css.
const BLUE  = '#1769E8';
const INK   = '#101F38';
const WHITE = '#FFFFFF';

const load = (f) => opentype.parse(fs.readFileSync(path.join(ROOT, 'public/fonts', f)).buffer);
const XB = load('Outfit-ExtraBold.ttf');
const BD = load('Outfit-Bold.ttf');

const r = (n) => Math.round(n * 1000) / 1000;

/* ---------- the "iz" monogram ----------
 * Ported verbatim from the Nuxt BrandLogo (components/BrandLogo.vue and
 * public/favicon.svg): drawn on a 64x64 grid as a stem, a dot and a folded z,
 * NOT outlined from the font. Hand geometry is why the mark survives a font
 * change and why both projects can ship the identical shapes.
 *
 * Edit these numbers and the Nuxt copies together, or the two drift apart. */
const GLYPH = {
  box: 64,
  // Bounding box of the three shapes below: the dot overhangs the stem on the
  // left (19 - 3.9) and sets the top edge, the z sets the right edge.
  x1: 15.1,
  y1: 16.1,
  x2: 49,
  y2: 48,
  // iFill paints the stem + dot, zFill the z. Pass the same colour for both to
  // get the single-colour mark.
  shapes: (iFill, zFill) =>
    '<rect x="15.5" y="26" width="7" height="22" fill="' + iFill + '"/>' +
    '<circle cx="19" cy="20" r="3.9" fill="' + iFill + '"/>' +
    '<path d="M27 26H49v6.5L37.8 41.5H49V48H27v-6.5L38.2 32.5H27Z" fill="' + zFill + '"/>',
};

GLYPH.width  = GLYPH.x2 - GLYPH.x1;
GLYPH.height = GLYPH.y2 - GLYPH.y1;
...
function markOnly(opts) {
  const size  = (opts && opts.size) || 200;
  const pad   = (opts && opts.pad)  || 0;
  const scale = size / GLYPH.height;

  // Trim the 64-grid margin so the emitted SVG is exactly the glyph, then
  // re-add the caller's pad - same contract the outlined version had.
  const body = (iFill, zFill) =>
    '  <g transform="translate(' + r(pad - GLYPH.x1 * scale) + ' ' + r(pad - GLYPH.y1 * scale) +
    ') scale(' + r(scale) + ')">' + GLYPH.shapes(iFill, zFill || iFill) + '</g>';

  return { body, width: GLYPH.width * scale + pad * 2, height: size + pad * 2 };
}
...
function icon(o) {
  o = o || {};
  const box        = o.box        || 512;
  const radius     = o.radius     === undefined ? 14 / GLYPH.box : o.radius; // Nuxt tile: rx=14 on a 64 box
  // Default is the glyph's own share of the 64 grid, so a 64-box tile comes
  // out as an exact copy of the Nuxt one: scale 1, no offset. Overrides are
  // for the full-bleed and maskable variants, which need a smaller glyph.
  const glyphRatio = o.glyphRatio === undefined ? GLYPH.width / GLYPH.box : o.glyphRatio;
  const bg         = o.bg;
  const fg         = o.fg;
  const bgStroke   = o.bgStroke || null;

  const fgAlt = o.fgAlt || fg;               // second glyph colour, for the two-tone favicon

  const scale = (box * glyphRatio) / GLYPH.width;
  const dx    = (box - GLYPH.width * scale) / 2 - GLYPH.x1 * scale;
  const dy    = (box - GLYPH.height * scale) / 2 - GLYPH.y1 * scale;
...
/* favicon - white plate with the two-tone glyph, matching the Nuxt
 * public/favicon.svg byte for byte. The installable-app icons below keep
 * the blue plate, which is the variant Nuxt ships as app-icon-blue.svg. */
fs.writeFileSync(path.join(ROOT, 'public/favicon.svg'), svg(Object.assign(icon({ box: 64, bg: WHITE, fg: INK, fgAlt: BLUE }), { title: 'izSuite' })));
